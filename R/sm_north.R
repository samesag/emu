...
#' @param location string indicating the symbol's location in the plot. Possible options: "topleft" (default), "bottomright", "bottomleft" and "topright".
#' @param scale a variable (or vector of variables) quoted that you want to change to binary.
#' @param symbol number between 1 and 3 to choose a symbol (1: black, 2: grey, 3: white)
#' @param x.min if data is not defined, number with the minimum x coordinate. Useful for ggmap.
#' @param x.max if data is not defined, number with the maximum x coordinate. Useful for ggmap.
#' @param y.min if data is not defined, number with the minimum y coordinate. Useful for ggmap.
#' @param y.max if data is not defined, number with the maximum y coordinate. Useful for ggmap.
#' @param anchor named vector with coordinates to control the symbol position. For location = "topright", anchor defines the coordinates of the symbol's topright corner and so forth. The x coordinate must be named as x and the y coordinate as y.
...
sm_north <- function (data = NULL, location = "topleft", scale = 0.1, symbol = 1,
          x.min, x.max, y.min, y.max, anchor = NULL)
{
  ...
  symbol <- png::readPNG(paste0(system.file("symbols", package = "emu"),
                                "/", symbol, ".png"))
